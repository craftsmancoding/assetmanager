...
class Asset extends xPDOObject
{
    ...
    public function all($args, $debug = false)
    {
        ...
        $limit = (int)$this->xpdo->getOption('limit', $args,
            $this->xpdo->getOption('assman.default_per_page', '', $this->xpdo->getOption('default_per_page')));
    ...
    public function dictateRelations(array $data, $this_id, $id_name = 'page_id', $join = 'PageAsset')
    {
        ...
        $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
            'Dictating Asset Relations for id ' . $this_id . ' with join ' . $join . ' : ' . print_r($data, true), '',
        ...
        foreach ($data as $r) {
            ...
            if (!$Rel->save()) {
                $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
                    'Error saving relation for asset_id ' . $Rel->get('asset_id') . ' and ' . $id_name . ' ' . $this_id,
                    '', __CLASS__, __FILE__, __LINE__);
            }
        }
        ...
        foreach ($existing as $e) {
            if (!in_array($e->get('asset_id'), $dictated)) {
                if (!$e->remove()) {
                    $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
                        'Error removing relation for asset_id ' . $e->get('asset_id') . ' and ' . $id_name . ' ' . $this_id,
                        '', __CLASS__, __FILE__, __LINE__);
                }
            }
        }
    ...
    public function fromFile($FILE, $force_create = false)
    {
        ...
        if (!$force_create) {
            if ($existing = $this->xpdo->getObject('Asset', array('sig' => $sig))) {
                $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
                    'Existing Asset found with matching signature: ' . $existing->get('asset_id'), '', __CLASS__,
                    __FUNCTION__, __LINE__);
                return $existing;
            }
        }

        $this->xpdo->log(\modX::LOG_LEVEL_DEBUG, 'Creating Asset from File in Storage Directory: ' . $storage_basedir,
        ...
        $this->set('title', $basename);
        $this->set('alt', '');
        $this->set('manual_url', '');
        $this->set('thumbnail_manual_url', '');
        $this->set('seq', 0);
        $this->set('sig', $sig);
        $this->set('size', $size);
        ...
        if (!@rename($src, $dst)) {
            $this->xpdo->log(\modX::LOG_LEVEL_ERROR, 'Failed to move asset file from ' . $src . ' to ' . $dst, '',
                __CLASS__, __FILE__, __LINE__);
            throw new \Exception('Could not move file from ' . $src . ' to ' . $dst);
        }

        $this->xpdo->log(\modX::LOG_LEVEL_DEBUG, 'Moved file from ' . $src . ' to ' . $dst, '', __CLASS__, __FILE__,
            __LINE__);
        ...
        if (!$this->save()) {
            $this->xpdo->log(\modX::LOG_LEVEL_ERROR,
                'Failed to save Asset. Errors: ' . print_r($this->errors, true) . ' ' . print_r($this->toArray(), true),
                '', __CLASS__, __FILE__, __LINE__);
            throw new \Exception('Error saving to database.');
        }

        $this->xpdo->log(\modX::LOG_LEVEL_DEBUG, 'Saved Asset: ' . print_r($this->toArray(), true), '', __CLASS__,
            __FUNCTION__, __LINE__);
        return $this;
    }
    ...
    public function getRelPath($fullpath, $prefix = null)
    {
        ...
        if (substr($fullpath, 0, strlen($prefix)) == $prefix) {
            ...
            $this->xpdo->log(\modX::LOG_LEVEL_ERROR, 'Prefix (' . $prefix . ') not found in path (' . $fullpath . ')',
                '', __CLASS__, __FILE__, __LINE__);
    ...
    public function getContentType($FILE)
    {
        ...
        if ($C = $this->xpdo->getObject('modContentType', array('mime_type' => $FILE['type']))) {
            $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
                'Content Type Found for mime-type ' . $FILE['type'] . ': ' . $C->get('id'), '', __CLASS__, __FILE__,
                __LINE__);
            return $C;
        }
        ...
        $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
            'Looking up content type for file ' . $FILE['name'] . ' by file extension', '', __CLASS__, __FILE__,
            __LINE__);
        $ext = ltrim(strtolower(strrchr($FILE['name'], '.')), '.');
        if ($C = $this->xpdo->getObject('modContentType', array('file_extensions' => '.' . $ext))) {
            $this->xpdo->log(\modX::LOG_LEVEL_DEBUG, 'Content Type Found for extension .' . $ext . ': ' . $C->get('id'),
                '', __CLASS__, __FILE__, __LINE__);
            return $C;
        }

        // Final chance: auto-create the content type        
        $this->xpdo->log(\modX::LOG_LEVEL_WARN, 'Unknown Content Type for file ' . $FILE['name'], '', __CLASS__,
            __FILE__, __LINE__);
        if ($this->xpdo->getOption('assman.autocreate_content_type')) {
            $this->xpdo->log(\modX::LOG_LEVEL_INFO,
                'Attempting to auto-create modContentType for file ' . $FILE['name'], '', __CLASS__, __FILE__,
                __LINE__);
    ...
    {
        //file_put_contents('/paas/c0010/www/core/cache/logs/debug.txt', print_r($array,true), FILE_APPEND);
        ...
        foreach ($array as $k => $v) {
            ...
            if (strtoupper(substr($k, -9)) == ':NOT LIKE') {
                $array[$k] = '%' . $v . '%';
            }
    ...
    public function preparePath($path, $umask = 0777)
    {
        ...
        if (file_exists($path)) {
            ...
            if (!is_dir($path)) {
                $this->xpdo->log(\modX::LOG_LEVEL_ERROR,
                    'Target directory must be a directory. File found instead: ' . $path, '', __CLASS__, __FILE__,
                    __LINE__);
            ...
            if (!@mkdir($path, $umask, true)) {
                $this->xpdo->log(\modX::LOG_LEVEL_ERROR,
                    'Failed to recursively create directory ' . $path . ' with umask ' . $umask, '', __CLASS__,
                    __FILE__, __LINE__);
        ...
        if (!touch($tmpfile)) {
            $this->xpdo->log(\modX::LOG_LEVEL_ERROR, 'Failed to write file to directory: ' . $path, '', __CLASS__,
                __FILE__, __LINE__);
    ...
    public function remove(array $ancestors = array())
    {
        $storage_basedir = $this->xpdo->getOption('assets_path') . rtrim($this->xpdo->getOption('assman.library_path'),
                '/') . '/';
        $this->xpdo->log(\modX::LOG_LEVEL_DEBUG,
            'Removing Asset ' . $this->getPrimaryKey() . ' with assets in storage_basedir ' . $storage_basedir, '',
            __CLASS__, __FILE__, __LINE__);
        ...
        if (file_exists($file)) {
            if (!unlink($file)) {
                $this->xpdo->log(\modX::LOG_LEVEL_ERROR,
                    'Failed to remove file asset for Asset ' . $this->getPrimaryKey() . ': ' . $file, '', __CLASS__,
                    __FILE__, __LINE__);
                throw new \Exception('Failed to delete asset file.');
            }
        } else {
            $this->xpdo->log(\modX::LOG_LEVEL_INFO,
                'File does not exist for Asset ' . $this->getPrimaryKey() . ': ' . $file . ' This could be because the file was manually deleted or because you did not pass the $storage_basedir parameter.',
                '', __CLASS__, __FILE__, __LINE__);
        }
