...
if (!is_numeric($asset_id)) {
    $modx->log(\modX::LOG_LEVEL_ERROR, 'Invalid input. Integer asset ID required. ' . print_r($scriptProperties, true),
        '', 'scale2h Output Filer');
    return;
}

if (!is_numeric($new_h)) {
    $modx->log(\modX::LOG_LEVEL_ERROR, 'Invalid option. Integer height required. ' . print_r($scriptProperties, true),
        '', 'scale2h Output Filer');
    return;
}
